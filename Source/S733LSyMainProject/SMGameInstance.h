...
UCLASS()
class S733LSYMAINPROJECT_API USMGameInstance : public UGameInstance
{
	GENERATED_BODY()
...
protected:
	...
	FName LobbyMapName; // Имя карты, которая будет загружаться во время старта онлайн сессии
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly)
	FName MainMenuMap; // Карта по умолчанию, карта, которая запускается при старте игры

protected:
	UPROPERTY(Replicated, EditAnywhere, Category = "Server settings")
	FName ServerName; // Имя сервера, который хостит игру (уникальный идентификатор)
	UPROPERTY(Replicated, EditAnywhere, Category = "Server settings")
	uint32 MaxPlayers;

	// функции для хоста игры
