...
EBTNodeResult::Type UBTTask_RandomPointAroundTarget::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	bool bIsFound = NavSys->GetRandomReachablePointInRadius(TargetActor->GetActorLocation(), Radius, NavLocation); // (вокруг какого эктора ищем, радиусб куда записывается результат)
	if (!bIsFound)
	{
		return EBTNodeResult::Failed;
	}
